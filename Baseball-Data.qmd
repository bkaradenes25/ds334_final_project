...
server <- function(input, output, session) {
  ...
  output$exit_velo_distance <- renderPlot({
    filtered_data <- base_graph()
    ggplot(filtered_data, aes(x = launch_speed, y = hit_distance_sc, color = events)) +
      geom_point() +
      labs(x = "Exit Velocity (mph)", y = "Distance (ft.)", title = paste("Exit Velocity vs Distance for", input$player_name)) +
      theme_minimal()
  })
}
